...
function Exams({ user }) {
    ...
    const fetchExams = async () => {
        try {
            const res = await axios.get('http://localhost:3050/api/exams', config);
            setExams(res.data);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleAddExam = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:3050/api/exams', { courseCode, title, date, type }, config);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:3050/api/exams/${id}`, config);
            fetchExams();
        } catch (err) {
            console.error(err);
        }
